...
- 路径：`C:\Users\niilo\AppData\Local\Temp\opencode\pi-native-pi-34`；分支 `issue-34-native-pi-rpc`，基于已合并 PR #38 的 `4e6cf6ad8a24dcaeaf93e3bcf84bc22a4e9e2b3a`。**#34 初始源码、文档、测试和必要截图已保存并推送至 `388f6727ef840614dc7e71f7bc44878d7d3392c2`；后续交互补验及服务边界增量需继续提交，不得重建、清理、重置或丢弃。**
...
| `pi-34-bundle-interaction/` + `pi-34-interaction-probe-10/` | **后续最新服务代码的**实际打包 exe/app.asar 复测通过初始链路及长流式自动跟随、手动上滚不抢滚动、返回最新；Pi 第二次 read 打开真实 hello.txt、原生两文件标签拖拽重排与 Side Pane 缩放、双 Pi 会话切换不重放、原生禁用 split 探针；报告明确 `http://127.0.0.1:64315/v1` 是该次运行的本地确定性模型测试端点，Pi 自己执行文件工具；`pageErrors=[]`、两次进程树无幸存 | 在线真实供应商、**原版滚动实际运行对照**、会话 split 可用性及安装器升级 |
...
受控模型服务是本地 OpenAI-compatible 测试端点，不是在线供应商推理；真实工具由**固定 Pi 进程**执行，不能写成仅 mock 工具。原生已完成轮次的“工作历史”默认折叠，所以原初脚本只查工具 DOM 判错；现按原生操作展开后工具卡实际可见，read 芯片点开原生 Side Pane 并验证正确文件。最新滚动/标签/会话/split 截图也已复制到 [#34 对照资料（待独立逐屏复核）](issue-34-parity/README.md)，与 #32 固定原版实际操作逐面关联并明确缺口。固定原版 #32 没有量测长流式自动/手动滚动，本轮试图在新临时副本离线安装以补原版实测，但 C: 约 2 GiB 可用空间触发 `ENOSPC`，已仅删除本轮失败副本并保留安装失败日志；**不能**将源码阅读或产品滚动通过写成原版滚动实测通过。脚本 `scripts/pi-native-gui-smoke.mjs` 支持开发预览，也可令 `NATIVE_PI_PACKAGED_EXE` 指向 `win-unpacked` 真正启动已打包入口；每次生成独立 profile、模型端点、操作报告和截图。
...
4. 逐项核对 #34 验收条件后做**独立** Standards/Spec 审查、修复复审、PR 与最新 Windows CI；不满足不得合并/关闭 #34。当前 `reviewer` 角色两次派遣均被模型策略立即拒绝（只能 GPT-5.6 Sol，现有未配置 Jev/合规模型），**没有独立审查结果，不能伪称通过**。需为独立审查提供合规 reviewer 运行环境。完成后暂停，等待用户确认后续范围，不自动领取其他票。
