...
- 脚本/配置预检后运行原命令 `pnpm --dir apps/zcode-cli lint`，保存原始 stdout、stderr、退出码。
...
86 个文件各限 **1 个 `eslint(max-lines)` error**；83 个文件有效行数不得超过上游。
另 3 个文件仅接受最终 `c896a8de6e351f61007efd9eec85bd8fe448c5e3` 的精确源码：
...
Turbo dry-run 及 `summary.json`（实际源码提交、逐项接受/失败、全部 warnings）。CI 应保存整个目录。
