...
最终源码的完整服务 suite **117/117**、完整 typecheck、6 个相关文件定向 lint **0 warnings / 0 errors**、退出脚本语法及 `git diff --check` 通过。按 CI 顺序重建，再从当前代码打出 Windows x64 NSIS 包 `release/issue-34-closeout/bundle-budget/Pi Agent IDE Preview-3.14.0-win-x64_TEST.exe`，依赖闭包与体积审计通过，**170.5 MiB**。直接运行同包 `win-unpacked/Pi Agent IDE Preview.exe` 的真实 GUI 报告在 `release/issue-34-closeout/gui-packaged-budget/`：Pi 8 次模型请求、`read`、Stop、重启恢复、滚动、文件预览与双会话通过；首次/最终退出分别识别 **9/10** 个本轮进程，均 `graceful=true`、`forced=[]`、`survivors=[]`；`pageErrors=[]`、隔离目录越界写入 `[]`。主会话目视查看包内深色恢复与文件预览截图，没有破图。最新原生完整 GUI smoke 见 `release/issue-34-closeout/gui-native-budget/`：**18 项动作 PASS**、48 个截图状态、242 个可见图片实例完整且尺寸非零、页面错误 0；该报告仍标注未覆盖的能力，不把 smoke 冒称完整 Issue 验收。Windows CI 仍待验证。

GUI 测试用隔离的本地确定性模型端点；Pi 是真实固定版本子进程并真实执行 `read`，不等同在线供应商推理或安装升级验收。当前仍须提交推送预算修复、等待**该提交**的 Windows push/PR CI 变绿，再续 Standards / Spec 全 PR 加增量复审；在复审通过前 PR 保持 Draft，#34 保持 OPEN。
