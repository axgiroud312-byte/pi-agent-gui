...
export class WorkspaceHost extends EventEmitter<{ change: [] }> {
  ...
  snapshot(): AppSnapshot {
    return structuredClone({ workspaces: this.workspaces, sessions: [...this.sessions.values()].map(item => item.snapshot), profile: this.profile, version: this.options.version });
  }
  ...
  async createSession(workspaceId: unknown): Promise<SessionSnapshot> {
    ...
    const snapshot: SessionSnapshot = {
      id: randomUUID(), workspaceId: workspace.id, generation: randomUUID(), phase: 'starting', piVersion: version, messages: [], diagnostics: [],
    };
    const client = new PiRpcClient({ executable: profile.executable, args: [...profile.args, '--mode', 'rpc'], cwd: workspace.path, env });
    ...
    try {
      ...
      if (typeof state.sessionFile === 'string') snapshot.sessionFile = state.sessionFile;
  ...
  private applyRecord(runtime: SessionRuntime, record: Record<string, unknown>): void {
    ...
    switch (record.type) {
      ...
      case 'auto_retry_end':
        ...
        break;
      case 'auto_compaction_start': snapshot.phase = 'compacting'; break;
      case 'auto_compaction_end': snapshot.phase = 'running'; break;
  ...
  async sendPrompt(id: unknown, value: unknown): Promise<void> {
    ...
    if (!runtime || runtime.closing) throw new Error('会话进程不可用，请新建会话');
    if (runtime.uncertain) throw new Error('此前请求状态待核实，请查看诊断并新建会话；未自动重放输入');
    if (busyPhases.has(runtime.snapshot.phase)) throw new Error('会话正在运行，请等待结束');
    if (runtime.snapshot.phase === 'exited') throw new Error('Pi 进程已退出，请新建会话');
  ...
  async dispose(): Promise<void> {
    ...
    await Promise.all([...this.sessions.values()].map(async runtime => { runtime.closing = true; await runtime.client.dispose(); }));
    await this.writes;
  }
}
