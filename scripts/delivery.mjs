...
for (const [field, required] of [
  ...
  // Final acceptance must not mask missing implementation coverage.
  const covered = new Set(tickets.filter(t => t.key !== 'release-acceptance').flatMap(t => t[field]));
...
for (const ticket of tickets) {
  ...
  if (!issue) {
    const url = gh(['issue', 'create', '--repo', repo, '--title', ticket.title, '--label', 'ready-for-agent', '--body-file', '-'], body);
