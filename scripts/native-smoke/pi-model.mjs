...
export async function startPiModel() {
  ...
  const server = createServer(async (req, res) => {
    ...
    const scenario = /PI_(?:TEXT|READ|STOP)/.exec(prompt)?.[0] ?? 'other';
    ...
    if (scenario === 'PI_READ' && toolResults.length === 0) {
    ...
    const response = scenario === 'PI_READ' ? 'PI_READ_COMPLETE' : scenario === 'PI_STOP'
