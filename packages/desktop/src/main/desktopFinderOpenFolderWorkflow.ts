...
const WORKFLOW_NAME = "Open in Pi Agent IDE.workflow";
const WORKFLOW_BUNDLE_ID = "io.github.axgiroud312-byte.pi-agent-ide.finder-open-workflow";
const WORKFLOW_VERSION = "5";
const SERVICES_MENU_LABELS: Record<Locale, string> = {
  "zh-CN": "在 Pi Agent IDE 中打开",
  "en-US": "Open in Pi Agent IDE",
};
...
if [ -n "$first" ]; then
  ...
  /usr/bin/open "pi-agent-ide://workspace/open?path=\${encoded}"
fi
`;
